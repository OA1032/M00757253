...
import tkinter as tk #GUi library as an abbreviated variable name 
...
def an7(): # Incomplete function 
    for colour in range(6):
        colour[colour+1 + rows*60] =(255,255,255)
        colour = colour + 1
        client.put_pixels(colour)
...
root.geometry("400x400") #Window size (x,y)
...
myButton7 = Button(root, text = "Animation 7" ,fg='white',bg='black',activebackground='orange',activeforeground='black',width=350, command = an7,state=DISABLED).pack()#defining the button parameters.This button is DISABLED due to the animation being incomplete
...
myButton12 = Button(root, text = "Arduino Control!" ,fg='white',bg='blue',width=350, command = arcomms).pack()#defining the button parameters. This button is DISABLED due to the animation being incomplete
